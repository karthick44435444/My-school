  ...
  const openDetail = async (c: any) => {
    ...
    try {
      ...
      if (mRes.ok) {
        ...
        maps = (d.classes || d.mappings || []).filter((x: any) => {
              ...
              {classes.map((c) => (
                ...
                >
                  <div>
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-lg border border-indigo-100 shadow-2xs group-hover:scale-105 transition-transform">
                          {c.name ? c.name.replace(/[^0-9a-zA-Z]/g, "").slice(0, 3) : "CLS"}
                        </div>
                        <div>
                          <h3 className="font-extrabold text-slate-900 text-base leading-tight group-hover:text-indigo-600 transition">
                            {c.name} - {c.section}
                          </h3>
                          <span className="text-[11px] font-semibold text-slate-400">Class &amp; Section</span>
                        </div>
                      </div>
                      ...
                      <div className="relative">
                        ...
                        {activeMenuId === c.id && (
                          <div
                            ...
                            className="absolute right-0 top-full mt-1 w-36 bg-white rounded-2xl shadow-xl border border-slate-100 py-1.5 z-20"
                          >
                            <button
                              type="button"
                              onClick={() => {
                              ...
                              className="w-full px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2"
                            >
                              <Pencil className="w-3.5 h-3.5 text-indigo-600" /> Edit Class
                            </button>
                            <button
                              type="button"
                              onClick={() => {
                            ...
                            >
                              <Trash2 className="w-3.5 h-3.5 text-red-600" /> Delete Class
                            </button>
                          </div>
                        )}
                      </div>
                    ...
                    {/* Class Teacher Info */}
                    <div className="bg-slate-50/80 rounded-2xl p-3 border border-slate-100 mb-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Class Teacher</span>
                        <span
                        ...
                        >
                          {c.classTeacherName ? "Assigned" : "Unassigned"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2.5">
                        <Avatar
                          name={c.classTeacherName || "Unassigned"}
                          photoUrl={c.classTeacherPhotoUrl}
                          size={28}
                        />
                    ...
                    {/* Quick Stats Pill */}
                    <div className="flex items-center justify-between text-xs text-slate-500 pt-1 pb-3 px-1">
                      ...
                      <div className="flex items-center gap-1.5 font-semibold">
                        ...
                        <span>{c.subjectTeacherCount ?? 0} Subjects</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center gap-2">
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        setMapSubjectName(c.classTeacherSubject || "");
                      }}
              ...
              <form onSubmit={updateClassAction} className="space-y-3.5">
                ...
                {editForm.classTeacherId && (
                  <div className="p-3 bg-indigo-50/80 rounded-2xl border border-indigo-100/90 flex items-start gap-2.5 mt-2">
                    ...
                    />
                    <label htmlFor="transferDataEdit" className="text-xs text-slate-700 cursor-pointer select-none">
              ...
              {mapTeacherId && (
                <div className="p-3 bg-indigo-50/80 rounded-2xl border border-indigo-100/90 flex items-start gap-2.5 mb-4">
                  <input
                    ...
                    className="mt-0.5 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer"
                  />
                  <label htmlFor="transferDataMap" className="text-xs text-slate-700 cursor-pointer select-none">
                    <span className="font-bold text-slate-900 block">
                      Transfer class information to new class teacher
                    </span>
                    <span className="text-[11px] text-slate-500 leading-tight block mt-0.5">
              ...
              {detailLoading ? (
                ...
                <>
                  ...
                  <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-100 flex items-center justify-between gap-3 mb-2">
                    ...
                    <Link
                      href={`/admin/students?className=${encodeURIComponent(classDetail.name)}&section=${encodeURIComponent(classDetail.section || "")}`}
                      onClick={() => setClassDetail(null)}
