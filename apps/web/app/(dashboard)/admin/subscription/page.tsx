  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden w-full max-w-full">
      ...
      <main className="lg:ml-64 pt-20 lg:pt-8 p-4 sm:p-6 lg:p-8 min-h-screen max-w-full overflow-x-hidden">
              ...
              {loading ? (
                ...
                <>
                  <h2 className="text-2xl sm:text-4xl font-extrabold tracking-tight">
                    {subscription?.currentPlan?.name || "SchoolVajo Starter"}
                  </h2>
                  <p className="text-sm text-indigo-100/90 mt-2 leading-relaxed">
                    {subscription?.currentPlan?.description || "Essential school management with vital campus tools for teachers, students, and parents."}
                  </p>
                </>
              )}
            ...
            <div className="flex flex-col items-start lg:items-end justify-center border-t lg:border-t-0 lg:border-l border-white/10 pt-4 lg:pt-0 lg:pl-8 shrink-0">
              ...
              {loading ? (
                ...
                <div className="text-3xl sm:text-5xl font-black mt-1 flex items-baseline gap-1">
                  <span>₹{subscription?.currentPlan?.price ?? 299}</span>
        ...
        {/* Plan Upgrade Selection Header & Duration Switcher */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-600 mt-0.5">
              Select validity period and plan tier to instantly recharge or upgrade.
            </p>
          </div>

          {/* Duration Selector Tabs */}
          <div className="flex items-center p-1.5 rounded-2xl bg-slate-200/80 border border-slate-200 self-start sm:self-auto shadow-inner">
            {DURATION_OPTIONS.map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  {opt.key === "1_YEAR" && (
                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.2 rounded-full font-extrabold">
                      Save 20%
                    </span>
                  )}
                </button>
        ...
        {/* Plans Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 items-stretch mb-10 w-full max-w-full pt-2">
          {PLAN_TIERS.map((tier) => {
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-3xl bg-white pt-8 pb-6 px-6 sm:pt-9 sm:pb-8 sm:px-8 border-2 flex flex-col justify-between transition-all overflow-visible ${
                  tier.id === "STARTER"
                    ? "border-indigo-600 ring-2 ring-indigo-600/20 shadow-xl"
                    : "border-slate-200 hover:border-indigo-300 shadow-md"
                }`}
              >
                {/* Top Badge */}
                <div
                ...
                <div>
                  <button
                    type="button"
                    onClick={() => setSelectedPlanToUpgrade(resolvedPlan)}
                    disabled={upgradingPlanId !== null}
                    className={`w-full py-3 px-4 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-sm cursor-pointer active:scale-95 ${
                      tier.id === "STARTER"
                  ...
                  >
                    {isCurrent ? (
                      <>
                        ...
                        <span>Recharge / Extend ({pricing.label})</span>
                      </>
                    ) : (
                      <>
