...
export default function AdminStudentsPage() {
  ...
  // Load class options and all-student counts for tabs
  ...
  // Fetch paginated data from API with server-side filters & search
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="lg:ml-64 pt-20 lg:pt-8 p-4 sm:p-6 lg:p-8 min-h-screen">
        ...
        {isSelectionMode && selectedIds.length > 0 && (
          <div className="mb-4 bg-indigo-50/90 border border-indigo-200 rounded-2xl p-3 px-4 flex flex-wrap items-center justify-between gap-3 shadow-xs animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-2.5">
              <span
                className="px-2.5 py-1 rounded-lg text-white font-bold text-xs shadow-xs"
                style={{ backgroundColor: theme }}
              >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl border border-indigo-200 bg-white text-indigo-700 text-xs font-semibold hover:bg-indigo-50 transition flex items-center gap-1.5 shadow-xs"
              >
                <ArrowRightLeft className="w-3.5 h-3.5 text-indigo-600" /> Change Class
              </button>
              <button
                type="button"
                onClick={() => setShowBulkDeleteConfirm(true)}
                className="px-3 py-1.5 rounded-xl bg-red-600 text-white text-xs font-semibold hover:bg-red-700 transition flex items-center gap-1.5 shadow-xs"
              >
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-xl text-slate-500 hover:text-slate-800 text-xs font-semibold hover:bg-slate-100 transition"
              >
                Clear
              </button>
            </div>
        ...
        <div className="bg-white rounded-2xl border overflow-x-auto shadow-xs">
          {loading ? (
            ...
            <>
              ...
              {/* Numbered Pagination with Prev/Next buttons */}
              <Pagination
                page={page}
