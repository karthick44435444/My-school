...
export default function AdminAnnouncementsPage() {
  const { user, loading } = useAuth(["ADMIN", "PRINCIPAL", "TEACHER"]);
    ...
    try {
      ...
      const res = await fetch("/api/announcements", {
        ...
        body: JSON.stringify({
          ...
          target: form.target,
          classes: classObjs.length > 0 ? classObjs : undefined,
          className: classObjs.length > 0 ? classObjs.map((c) => (c.section ? `${c.className}-${c.section}` : c.className)).join(", ") : undefined,
        }),
      });
