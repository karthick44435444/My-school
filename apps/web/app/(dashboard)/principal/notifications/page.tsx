...
function hrefForNotification(n: any): string | null {
  ...
  const meta = n.meta || {};
  if (type === "MARKS" || type === "EXAM") {
    const id = meta.examId || meta.itemId;
    return id ? `/principal/exams?highlight=${id}` : "/principal/exams";
  }
  if (type === "LEAVE" || type === "ATTENDANCE") {
    const q = new URLSearchParams();
    if (meta.date) q.set("date", meta.date);
    if (meta.date) q.set("highlight", meta.date);
    return `/principal/attendance${q.toString() ? `?${q}` : ""}`;
  }
  if (type === "HOMEWORK") {
    const id = meta.itemId || meta.homeworkId;
    return id ? `/principal/homework?highlight=${id}` : "/principal/homework";
  }
  if (type === "ANNOUNCEMENT") {
    const id = meta.itemId || meta.announcementId;
    return id ? `/principal/announcements?highlight=${id}` : "/principal/announcements";
  }
  return null;
}
      ...
      <main className="lg:ml-64 pt-20 lg:pt-8 p-4 sm:p-6 lg:p-8 min-h-screen">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Updates, alerts, administration notices, and school communications
            </p>
          </div>
          <button
            type="button"
            onClick={markAll}
            className="self-start sm:self-auto text-xs font-bold text-indigo-600 hover:bg-indigo-50 px-3.5 py-1.5 rounded-xl border border-indigo-200 transition cursor-pointer"
          >
            Mark all read
          </button>
        </div>
        ...
        {busy ? (
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 p-16 text-center shadow-xs">
            ...
            <p className="text-xs text-slate-400 mt-1">You will receive alerts here for school events, announcements, and reports</p>
          </div>
        ) : (
            ...
            {list.map((n, index) => {
              ...
              return (
                <div key={n.id}>
                  ...
                  <button
                    onClick={() => onClick(n)}
                    className={`w-full text-left bg-white rounded-3xl border p-5 relative pb-10 shadow-xs hover:shadow-md transition-all cursor-pointer ${
