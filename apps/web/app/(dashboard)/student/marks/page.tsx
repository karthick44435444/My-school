...
export default function StudentMarksPage() {
  ...
  const loadMarks = async (currentPage = page, query = debouncedQuery) => {
    if (!user) return;
    setBusy(true);
    try {
      let url = `/api/exams?studentId=${encodeURIComponent(user.id)}&page=${currentPage}&limit=20`;
      if (query.trim()) {
      ...
      if (res.ok) {
        ...
        for (const m of rawMarks) {
          const mid = m.id || `${user.id}-${m.examId || m.subject || ""}`;
          fetch("/api/read", {
            method: "POST",
  ...
  useEffect(() => {
    if (user) {
      loadMarks(page, debouncedQuery);
    }
  }, [user?.id, page, debouncedQuery]);
      ...
      <main className="lg:ml-64 pt-20 lg:pt-8 p-4 sm:p-6 lg:p-8 min-h-screen">
        ...
        {busy ? (
          <div className="p-16 flex flex-col items-center justify-center text-slate-500 gap-3">
            <Loader2 className="w-8 h-8 animate-spin text-indigo-600" />
            <span className="text-sm font-medium">Loading published results...</span>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 p-16 text-center shadow-xs">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {searchQuery ? "Try refining your search filter" : "Check back later once your teachers publish marks"}
            </p>
          </div>
        ) : (
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              return (
                <motion.div
                  ...
                  className="bg-white rounded-3xl border border-slate-200/90 p-6 shadow-xs hover:shadow-lg hover:border-indigo-200 transition-all flex flex-col justify-between"
                >
                  <div>
                    {/* Card Top: Badges */}
