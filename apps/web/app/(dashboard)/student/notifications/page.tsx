...
function hrefForNotification(n: any): string | null {
  ...
  if (type === "MARKS") {
    ...
    return id ? `/student/marks?highlight=${id}` : "/student/marks";
  }
  if (type === "LEAVE" || type === "ATTENDANCE") {
    const q = new URLSearchParams();
    if (meta.date) q.set("date", meta.date);
    if (meta.date) q.set("highlight", meta.date);
    return `/student/attendance${q.toString() ? `?${q}` : ""}`;
  }
  if (type === "HOMEWORK") {
    const id = meta.itemId || meta.homeworkId;
    return id ? `/student/homework?highlight=${id}` : "/student/homework";
  }
  if (type === "ANNOUNCEMENT") {
    const id = meta.itemId || meta.announcementId;
    return id ? `/student/announcements?highlight=${id}` : "/student/announcements";
  }
  return null;
}
      ...
      <main className="lg:ml-64 pt-20 lg:pt-8 p-4 sm:p-6 lg:p-8 min-h-screen">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Updates, alerts, marks announcements, and exam schedules
            </p>
          </div>
          <button
            type="button"
        ...
        {busy ? (
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 p-16 text-center shadow-xs">
            ...
            <p className="text-xs text-slate-400 mt-1">You will receive alerts here for marks, exams, and attendance</p>
          </div>
        ) : (
