...
export type CreateUserRole = "STUDENT" | "TEACHER" | "PRINCIPAL";
...
type Props = {
  ...
  open: boolean;
  onClose: () => void;
  onCreated: (credentials: any) => void;
  title?: string;
};
...
export default function CreateUserForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm(emptyForm());
      setErrors({});
      onCreated(data.credentials || data);
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
      <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-slate-100">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0 sticky top-0 z-10">
          <h2 className="font-bold text-lg text-slate-900">{title || `Add ${ROLE_LABEL[role]}`}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 min-h-0">
          ...
          >
            ...
            {saving ? "Creating..." : `Create ${ROLE_LABEL[role]}`}
          </button>
        </form>
        </div>
      </div>
